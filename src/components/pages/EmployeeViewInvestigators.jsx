...
function ManageInvestigators() {
  ...
  const handleFetch = async () => {
    try {
      const res = await axios.get('http://127.0.0.1:8000/account/view_all_users/');
      if (res.data) {
  ...
  const handleDelete = async (id) => {
    ...
    if (conf) {
      try {
        const res = await axios.delete('http://127.0.0.1:8000/account/delete_user/'+id);
  ...
  return (
    <>
      <h1 className='text-center text-black text-xl capitalize mb-4'>All Investigators</h1>
        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
              ...
              <th scope="col" className="px-6 py-3">Intitution</th>
          ...
          <tbody>
            {userData.length > 0 ? (
              userData.map((user, index) => (
                <tr key={index} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                  ...
                  <td className="px-6 py-4">
                    ...
                    <Link to={`/admin/edituser/${user.id}`} className=' px-4 py-2'>Connect</Link>
              ...
              <tr>
                <td colSpan="5" className="px-6 py-4 text-center">No Investigator found</td>
              </tr>
            )}
          </tbody>
        </table>
